...
# Dual-mode embedding support (TD-015):
# - DATABASE_TYPE=postgres + pgvector installed → uses Vector(1536) for native similarity search
# - Otherwise → uses JSON (SQLite-compatible fallback)
...
_USE_PGVECTOR = (_DATABASE_TYPE == "postgres") and _HAS_PGVECTOR
...
class Memory(Base):
    ...
    task_id = Column(String(255), nullable=True, index=True)  # TD-013: Task-scoped memory retrieval
    ...
    # Dual-mode: Vector(1536) for PostgreSQL+pgvector, JSON for SQLite (TD-015)
    embedding = (
        Column(PgVector(1536), nullable=True)
        if _USE_PGVECTOR
    ...
    pattern_examples = relationship(
        "PatternExample", back_populates="memory", cascade="all, delete-orphan"
    )
