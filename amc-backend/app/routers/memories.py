...
async def create_memory(
    ...
    # Create memory instance - need workspace_id from somewhere
    # For now, using a default workspace
    db_memory = Memory(
...
async def list_memories(
    ...
    # Get total count
    count_query = select(Memory.id).where(Memory.deleted_at.is_(None))
    if agent_id:
...
async def update_memory(
    ...
    if 'content' in update_data:
        ...
        memory.content_hash = hashlib.sha256(update_data['content'].encode('utf-8')).hexdigest()
